...
namespace
{
   ...
   {
      DM_STATIC_ASSERT(sizeof(Cartridge::Header) == kHeaderSize);
      DM_ASSERT(data.size() >= kHeaderOffset + kHeaderSize);
...
// static
std::unique_ptr<Cartridge> Cartridge::fromData(std::vector<uint8_t> data, std::string& error)
{
