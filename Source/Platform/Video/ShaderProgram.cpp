...
void Uniform::commit()
{
   ...
   switch (type)
   {
      ...
      case GL_FLOAT_MAT4:
         ...
         break;
      default:
         DM_ASSERT(false, "Invalid uniform type: %d", type);
         break;
   }
...
{
   DM_ASSERT(shaders.size() >= 2, "Need at least two shaders to link: %lu", shaders.size());
   ...
   for (GLint i = 0; i < numUniforms; ++i)
   {
      ...
      if (length < 1 || size < 1)
      {
         DM_LOG_WARNING("Unable to get active uniform: " << i);
      }
      else
      {
...
{
   DM_ASSERT(hasUniform(name), "Uniform with given name doesn't exist: %s", name.c_str());
