...
{
   DM_ASSERT(vertices || numVertices == 0, "numVertices > 0, but no vertices provided");
   DM_ASSERT(indices || numIndices == 0, "numIndices > 0, but no indices provided");
   DM_ASSERT(dimensionality >= 1 && dimensionality <= 4, "dimensionality must be between 1 and 4 (inclusive)");
