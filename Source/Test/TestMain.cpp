...
namespace
{
   ...
   void runProfileInPath(std::string path, float time)
   {
      std::vector<uint8_t> cartData = IOUtils::readBinaryFile(path);
      std::string error;
      if (UPtr<GBC::Cartridge> cartridge = GBC::Cartridge::fromData(std::move(cartData), error))
      {
         UPtr<GBC::GameBoy> gameBoy = std::make_unique<GBC::GameBoy>();
...
int main(int argc, char *argv[])
{
   if (argc > 2)
   {
      ...
      if (type == "-tests")
      {
         if (argc > 3)
         {
            static const float kDefaultTestTime = 10.0f;
      ...
      else if (type == "-profile")
      {
         ...
         runProfileInPath(argv[2], time);
         return 0;
      }
   }

   std::printf("Usage: %s [-tests carts_path output_path [test_time]] [-profile cart_path [profile_time]]\n", argv[0]);
   return 0;
}
